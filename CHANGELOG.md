...
### Changed
- **Mobile header consolidated.** On the mobile build the top toolbar is gone: the horizontally-scrolling KPI strip became a **Net-worth hero** (the headline number + its change over the range, with Min balance and Net/mo as supporting stats), and the theme toggle + account/data actions collapsed into a single **☰** menu that opens a settings bottom sheet (Account · Appearance · Data). Desktop keeps its topbar and the strip.
- **Mobile menus unified as bottom sheets.** Every "menu" you open on mobile is now the same bottom-sheet pattern (grabber, sections, green-active chips): the Projection **Range** menu, the Graph **Options** menu, and the Accounts/Flows **Filters** menus all moved from inline-expanding collapsibles to sheets, alongside the existing plan-hub and settings sheets. Short toggles use chips; long lists (sort/group/account) use selects. Desktop keeps its inline collapsible toolbars.
...
- **Graph reoriented to a vertical layered scroll.** The flow diagram's layers now stack top-to-bottom as a tall, scrollable canvas instead of horizontal columns, with each node pinned to its layer's band. Nodes are enlarged with uniform sizing and legible labels — the canvas now accounts for device pixel ratio, so text is sized in real terms rather than rendering tiny on high-density screens.
- **Graph layers reordered into a downward money waterfall.** Income now sits at the top, and the layers are ordered **Income → Deductions → Assets → Debts → Expenses** so every cross-layer flow points downward (income meets friction on the way down; assets pay down into debts; everything drains to expenses). The old single "Accounts" layer was split into **Assets** (what you own) and **Debts** (what you owe) so debt payments flow down rather than sideways. Gross-vs-net is carried by the flow amounts (net pay into checking vs. separate deduction flows), not the layer order.
- **Graph layout is now computed and static, with a focus interaction.** The continuous force simulation is gone (it caused nodes to wander). Positions are computed once and ease into place on open: each band centres its most-connected node — so the heavy flow runs down a central **spine** — and fans lower-degree nodes out to the edges (barycentre only breaks ties between equal-degree nodes). Edges route down out of each node and into the *top* of the next, doing their horizontal shift in the gap between layers, so they read as near-vertical trunks instead of diagonals that clip neighbours. The resting view is a calm uniform bundle (no amount labels, arrowheads, or width-by-amount); **tap a node** to light up just it, its edges (with amounts), and its direct neighbours while everything else fades to a ghost; **tap a lit neighbour** to traverse onto it. Dragging was removed in favour of tap-to-focus + swipe-to-scroll.
