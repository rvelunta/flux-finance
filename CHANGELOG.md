...
- **Graph layout is now computed and static, with a focus interaction.** The continuous force simulation is gone (it caused nodes to wander). Positions are computed once via a barycenter pass — connected nodes line up vertically and crossings drop — and ease into place on open. The resting view is a calm uniform bundle (no amount labels, arrowheads, or width-by-amount); **tap a node** to light up just it, its edges (with amounts), and its direct neighbours while everything else fades to a ghost; **tap a lit neighbour** to traverse onto it. Dragging was removed in favour of tap-to-focus + swipe-to-scroll.
